...
describe('Fazer busca de destinos', () => {
  ...
  it('Fazer busca de destinos IDA com 1 passageiro', () => {
    ...
    cy.get('#search-button', {timeout: 10000}).should('be.visible').click()
    // cy.wait(5000);
    cy.selecionarPassagemAleatoria1({ timeout: 10000 })
  ...
  //   cy.url().should('include', '/disponibilidade', { timeout: 20000 }).should('be.visible');
  //   cy.selecionarPassagemVolta('CAMA')
  //   cy.get('#buyer-check-1', { timeout: 30000 }).should('exist').and('be.visible');
  //   cy.get('#buyer-check-1').click({ force: true })
  //   cy.get('#btn-proceed').should('be.visible').and('not.be.disabled').click();
  //   cy.contains('Escolha o seu assento', { wait: 1000}).should('be.visible');
  //   cy.selecionarAssentoAleatorio1({ timeout: 10000 });
  //   cy.get('#btn-proceed').should('be.visible').click()
  //   cy.contains('Resumo da compra', { wait: 3000 }).should('be.visible');
  //   cy.selecionarAssentoAleatorio({ timeout: 5000 });
  //   cy.get('#btn-proceed').should('be.visible').click();
  //   cy.url().should('include', '/pagamento')
  //   cy.get('#tab-pix').click() 
